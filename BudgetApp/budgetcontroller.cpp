...
{
    // want to:
    // Find this transaction in the budget object
    // Decide if we are replacing or adding a new one...?
    // - Replacing
    //      Search budget transactions until a matching transaction, delete/update the match and make sure this info is emitted to the gui thread
    //          What can define a 'match'? Same title, description, and type?
    //          If so, How will I change titles, description, etc?
